...
class LinkedList {
  ...
  findKthFromEnd(k) {
    // La idea es tener 2 punteros: slow y fast
    // estaran separados k posiciones.
    // Cuando el fast node llegue al final retornaremos al slow
    let slow = this.head;
    let fast = this.head;
    ...
    // Una vez estan separadaos se procede a mover ambos punteros
    while (fast !== null) {
      slow = slow.next;
      fast = fast.next;
    }
